...
public class IntegerPathParamInjector implements ValueInjector {

	private boolean encode;
	private String paramName;
	private Integer defaultValue = null;

	public IntegerPathParamInjector(Class<?> type, Type genericType, AccessibleObject target, IntegerPathParam pathParam, String defaultValue, boolean encode, Annotation[] annotations, ResteasyProviderFactory factory) {
		this.encode = encode;
	...
	public Object inject(HttpRequest request, HttpResponse response) {
		String integerStr = request.getUri().getPathParameters(!encode).getFirst(paramName);
		if (integerStr == null) {
			return defaultValue;
		}
		try {
